...
def create_master_frames(bias_dir, dark_dir, plot_out_dir):
    ...
    if dark_files:
        ...
        if master_bias is not None:
            ...
            # Fallback if there are darks but no biases
            master_dark = np.median(dark_stack, axis=0)
        print(f"[*] Created Master Dark from {len(dark_files)} frames.")
        
        # Save Dark PNG
        plt.figure(figsize=(8, 8))
        vmin, vmax = np.percentile(master_dark, 1), np.percentile(master_dark, 99.9) # 99.9 to show hot pixels
...
# --- 3. MAIN PIPELINE ---
def process_fits_directory(input_dir, output_csv_dir, output_plot_dir, bias_dir, dark_dir, plot_out_dir, sigma_psf=2.0, threshold_factor=5.0, skip_bg_sub=False):
    # Ensure output directories exist
    os.makedirs(output_csv_dir, exist_ok=True)
    os.makedirs(output_plot_dir, exist_ok=True)

    # Note: Still keeping the [:3] limit for your quick testing!
    fits_files = sorted(glob.glob(os.path.join(input_dir, "*.fit")))[:1]
    if not fits_files:
    ...
    for frame_idx, filepath in enumerate(fits_files):
        ...
        if skip_bg_sub:
            img_clean = img_calibrated  # Bypass subtraction
        else:
            img_clean, background_est = levesque_process(img_calibrated, sigma_psf=sigma_psf)

            # --- SAVE DIAGNOSTIC COMPARISON (For the first 3 frames) ---
        if frame_idx % 20 == 0:
        ...
        # --- THE FIX: Prevent Microscopic Noise Collapse ---
        # Real sensors always have a few ADUs of read noise. 
        # If the math calculates something near zero, force a realistic minimum!
        MIN_NOISE_FLOOR = 1.0 # You can tweak this between 0.5 and 5.0 based on your camera
        if robust_sigma < MIN_NOISE_FLOOR: 
            robust_sigma = MIN_NOISE_FLOOR 
        # ---------------------------------------------------
        
        threshold = threshold_factor * robust_sigma
        
        # Add a print statement so you can see exactly what the math is doing
        print(f"Peak Score: {np.max(score_map):.2f} | Noise Floor: {robust_sigma:.4f} | Final Threshold: {threshold:.2f}")
...
if __name__ == "__main__":
    INPUT_FITS_DIR = "data/multi_target" 
    OUTPUT_CSV_DIR = "results/pipeline_output"
    OUTPUT_PLOT_DIR = "results/detection_plots" 
    PLOT_OUT_DIR = "results/calibration_plots" # Where calibration PNGs go
    BIAS_DIR = r"data\bias"
    DARK_DIR = r"data\dark"
